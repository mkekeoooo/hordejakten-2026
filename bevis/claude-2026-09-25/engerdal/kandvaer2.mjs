...
const cands = [];
for (const f of ['../ef/strict.md', '../ef/strict_solor.md']) if (fs.existsSync(f)) for (const l of fs.readFileSync(f, 'utf8').trim().split('\n')) { const c = l.split('|').map(s => s.trim()); const [la, lo] = c[2].split(',').map(Number); cands.push([c[1], la, lo, c[3], c[5]]); }
for (const [n, la, lo] of [['gml 1 Myklebysæterveien', 61.3995, 11.0316], ['gml 2 Madsskardveien', 61.4443, 11.1234], ['gml S1 Messelt', 61.4571, 10.8362], ['gml 4 Madsskardveien øst', 61.4518, 11.1428], ['gml A1-G13', 61.44856, 10.98472]]) cands.push([n, la, lo, '', '']);
