...
if ((process.argv[1] || "").endsWith('trinnA.mjs')) {
  const [e0, n0] = toUTM(61.72, 11.75), [e1, n1] = toUTM(62.05, 12.15), [e2] = toUTM(61.72, 12.15), [e3] = toUTM(62.05, 11.75);
  ...
  const segs = []; for (const w of JSON.parse(fs.readFileSync('roads.json', 'utf8')).elements) { const g = w.geometry.map(p => toUTM(p.lat, p.lon)); for (let i = 1; i < g.length; i++) segs.push([...g[i - 1], ...g[i]]); }
