...
gl.forEach((r, i) => r.forEach((v, j) => { const o = go[i][j]; if (v >= 57.8 && v <= 64.6 && o >= 4.4 && o <= 14.2) { y0 = Math.min(y0, i * S); y1 = Math.max(y1, i * S); x0 = Math.min(x0, j * S); x1 = Math.max(x1, j * S); } }));
y0 = Math.max(0, y0 - S); x0 = Math.max(0, x0 - S); y1 += S; x1 += S; const ST = 2; const H = Math.floor((y1 - y0) / ST) + 1, W = Math.floor((x1 - x0) / ST) + 1;
console.log(`region Yc ${y0}–${y1}, Xc ${x0}–${x1} (${W}×${H} celler)`);
const LAT = rows(await get(`lat[${y0}:${ST}:${y1}][${x0}:${ST}:${x1}]`), 'lat.lat'), LON = rows(await get(`lon[${y0}:${ST}:${y1}][${x0}:${ST}:${x1}]`), 'lon.lon');
...
async function field(hhmm) { const I = idxNear(hhmm); const acc = new Float32Array(W * H).fill(0), cnt = new Uint8Array(W * H);
  for (const i of I) { const f = rows(await get(`lwe_precipitation_rate[${i}:1:${i}][${y0}:${ST}:${y1}][${x0}:${ST}:${x1}]`), 'lwe_precipitation_rate.lwe_precipitation_rate');
...
fs.writeFileSync('regnskann_norge.json', JSON.stringify({ y0, x0, W, H, THR, LAT: LAT.flat(), LON: LON.flat(), score: score.map(s => Array.from(s, v => Number.isFinite(v) ? +v.toFixed(3) : null)) }));
// punktoppslag
