...
const steder = [['5a Jernvinneveien', 61.43511, 11.14349], ['5b Jernvinneveien', 61.43444, 11.13907], ['5c Jernvinneveien', 61.43531, 11.13483], ['A1 Birkebeinerveien', 61.4487, 10.9775],
  ['1 Myklebysæterveien (kontroll)', 61.3995, 11.0316]];
...
for (const [navn, la, lo] of steder) {
  ...
  for (let az = 188; az <= 250; az += 2) { const [ux, uy] = gdir(la, lo, az);
  ...
  console.log(`${navn.padEnd(32)} ${n} punkter i synsfeltet · laveste nødvendige høyde for sol kl. 07.50: ${minZ.toFixed(1)} m over bakken (ved ${hvor[0]}°, ${hvor[1]} m)`);
}
